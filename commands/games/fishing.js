...
module.exports = class Command extends Commando.Command {
	...
	async run(message) {
		...
		this.client.discordTogether.createTogetherCode(channel.id, 'fishing').then(async (invite) => {
			const embed = new Discord.MessageEmbed().setAuthor('Fishington.io').setColor(this.client.config.discord.accentColor).setTimestamp().setTitle(`Click here to join`).setURL(invite.code);
